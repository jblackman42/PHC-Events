...
router.get("/files", (req, res) => {
  fs.readdir(path.join(__dirname, "../dist"), (err, files) => {
    res
      .send(files.filter((file) => file !== "styles"))
      .status(200)
      .end();
  });
});
...
router.post("/staff", async (req, res) => {
  ...
  if (!Contact_ID_List) {
    res.status(403).send({
      msg: "Procedure or function 'api_Widget_GetStaff' expects parameter '@Contact_ID_List', which was not supplied.",
    });
  }
...
router.get("/sermons", async (req, res) => {
  ...
  if (!SeriesID) {
    res.status(403).send({
      msg: "Procedure or function 'api_Widget_GetSeries' expects parameter '@Sermon_Series_ID', which was not supplied.",
    });
  }
...
router.get("/group-register", async (req, res) => {
  ...
  if (!formGUID)
    return res.status(400).send({ err: "no form guid found" }).end();
...
router.get("/ministry-question", async (req, res) => {
  ...
  if (!ministryQuestionID)
    return res.status(400).send({ err: "no ministry question id" }).end();
...
router.get("/expirations", async (req, res) => {
  ...
  if (!guid)
    return res.status(400).send({ err: "Missing Parameter: guid" }).end();
