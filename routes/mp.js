...
router.get("/user-tasks", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!userID)
    return res.status(400).send({ error: "no userID parameter" }).end();
...
router.get("/event", ensureApiAuthenticated, async (req, res) => {
  ...
  const event = await axios({
    ...
    params: {
      ...
      $select:
        "Event_ID, Event_Title, Event_Type_ID_Table.[Event_Type], Congregation_ID_Table.[Congregation_Name], Location_ID_Table.[Location_Name], Meeting_Instructions, Events.[Description], Program_ID_Table.[Program_Name], Primary_Contact_Table.[Display_Name], Participants_Expected, Minutes_for_Setup, Event_Start_Date, Event_End_Date, Minutes_for_Cleanup, Cancelled, Featured_On_Calendar",
    },
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${await getAccessToken()}`,
...
router.get("/events", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!monthStart || !monthEnd)
    return res.status(400).send({ err: "you suck" }).end();
...
router.get("/places", ensureApiAuthenticated, async (req, res) => {
  const places = await axios({
    ...
    params: {
      ...
      $select:
        "Rooms.Room_ID, Rooms.Room_Name, Rooms.Building_ID, Building_ID_Table.[Building_Name], Building_ID_Table_Location_ID_Table.[Location_ID], Building_ID_Table_Location_ID_Table.Location_Name",
    },
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${await getAccessToken()}`,
  ...
  const locationsArr = locations.map((location, locationID) => {
    const buildings = [
      ...new Set(
        places
          .filter((place) => place.Location_Name == location)
          .map((place) => place.Building_Name)
      ),
    ];
    return {
      Location_Name: location,
      Location_ID: places.filter((place) => place.Location_Name == location)[0]
        .Location_ID,
      Buildings: buildings.map((building, buildingID) => {
        const rooms = [
          ...new Set(
            places
              .filter((place) => place.Building_Name == building)
              .map((place) => place.Room_Name)
          ),
        ];
        return {
          Building_Name: building,
          Building_ID: places.filter(
            (place) => place.Building_Name == building
          )[0].Building_ID,
          Rooms: rooms.map((room, roomID) => {
            return {
              Room_Name: room,
              Room_ID: places.filter((place) => place.Room_Name == room)[0]
                .Room_ID,
            };
          }),
        };
...
router.post("/overlapped-rooms", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!dates || !dates.length)
    return res
      .status(400)
      .send({ err: "no startDate or endDate provided" })
      .end();

  const filterDateString = dates
    .map((date) => {
...
router.get("/event-equipment", ensureApiAuthenticated, async (req, res) => {
  ...
  const events = await axios({
    ...
    params: {
      ...
      $select:
        "Event_Equipment.[Equipment_ID], Equipment_ID_Table.[Equipment_Name], Event_Equipment.[_Approved], Event_Equipment.[Quantity]",
    },
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${await getAccessToken()}`,
...
router.get("/event-services", ensureApiAuthenticated, async (req, res) => {
  ...
  const events = await axios({
    ...
    params: {
      $filter: `Event_ID = ${eventId} AND Cancelled = 0`,
      $select:
        "Event_Services.[Service_ID], Service_ID_Table.[Service_Name], Service_ID_Table.[Description], Event_Services.[_Approved]",
    },
    headers: {
      "content-type": "application/json",
      authorization: `Bearer ${await getAccessToken()}`,
...
router.get("/primary-contacts", ensureApiAuthenticated, async (req, res) => {
  const users = await axios({
    ...
    params: {
      $filter: `User_Group_ID=76`,
      $select:
        "User_ID_Table.[Contact_ID], User_ID_Table_Contact_ID_Table.[Display_Name]",
      $distinct: true,
...
router.get("/equipment", ensureApiAuthenticated, async (req, res) => {
  const equipment = await axios({
    ...
    params: {
      ...
      $select:
        "Equipment_ID, Equipment_Name, Date_Acquired, Equipment_Type_ID_Table.[Equipment_Type], Quantity",
...
router.post("/events", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!events || !events.length)
    return res.status(400).send({ err: "no event provided" }).end();
...
router.post("/event-rooms", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!roomsToBook || !roomsToBook.length)
    return res.status(400).send({ err: "no rooms to book provided" }).end();
...
router.post("/event-services", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!servicesToBook || !servicesToBook.length)
    return res.status(400).send({ err: "no rooms to book provided" }).end();
...
router.post("/event-equipment", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!equipmentToBook || !equipmentToBook.length)
    return res.status(400).send({ err: "no equipment to book provided" }).end();
...
router.post("/event-sequences", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!Event_IDs || !Table_Name)
    return res
      .status(400)
      .send({ err: "no event id or table name provided" })
      .end();
...
router.post("/event-groups", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!eventGroupsToBook || !eventGroupsToBook.length)
    return res.status(400).send({ err: "no groups to book provided" }).end();
...
router.post("/event-room-groups", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!eventRoomGroupsToBook || !eventRoomGroupsToBook.length)
    return res
      .status(400)
      .send({ err: "no event room groups to book provided" })
      .end();
...
router.post("/sermon-link", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!sermon_links || !sermon_links.length)
    return res.status(400).send({ err: "no sermon_links provided" }).end();
...
router.post("/announcements", ensureApiAuthenticated, async (req, res) => {
  ...
  if (!announcements || !announcements.length)
    return res.status(400).send({ err: "no announcements provided" }).end();
...
router.get("/current-guide", async (req, res) => {
  try {
    const { Link_URL } = await axios({
      ...
      params: {
        $filter:
          "Link_Type_ID = 4 AND Sermon_ID_Table.[Sermon_Date]  < GETDATE()",
...
router.get("/my-contributions/:guid", async (req, res) => {
  ...
  try {
    const [{ Contact_ID }] = await MinistryPlatformAPI.request(
      "get",
      "/tables/Contacts",
      { $select: "Contact_ID", $filter: `Contact_GUID='${Contact_GUID}'` },
      {}
    );
    const [{ Statement_ID }] = await MinistryPlatformAPI.request(
      "get",
      "/tables/Contribution_Statements",
      {
        $select: "Statement_ID",
        $filter: `Contact_Record=${Contact_ID}`,
        $orderby: "Last_Statement_File DESC",
      },
      {}
    );
    await MinistryPlatformAPI.request(
      "get",
      "/tables/Contribution_Statements",
      { $filter: "Contact_Record=94810", $orderby: "Last_Statement_File DESC" },
      {}
    );
    const [{ UniqueFileId }] = await MinistryPlatformAPI.request(
      "get",
      `/files/Contribution_Statements/${Statement_ID}`,
      {},
      {}
    );
